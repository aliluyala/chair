...
<script type="text/javascript">
	function submitFormByProxy(){
		if(!$('#proxyContent').form('validate')){
			$.messager.alert('提示','代理表单还未填写完成!');
			return ;
		}
		$.post('/<%=chair%>/manager/edit',$("#proxyContent").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','编辑代理成功!');
				$('#proxyEdit').window('close');
				$("#proxyList").datagrid("reload");
//				clearFormByProxy();
			}else{
				$.messager.alert('提示','编辑代理失败!');
			}
		});
	}
